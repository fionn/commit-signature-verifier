...
func TestSSHSignature(t *testing.T) {
	tests := []struct {
		...
		message       []byte
		signature     []byte
		allowedSigner xssh.AllowedSigner
		namespace     string
		timestamp     time.Time
	}{
		{"GoodSignature", message, signature, *allowedSigner, namespace, timestamp},
		{"BadSignature", message, []byte("yolo"), *allowedSigner, namespace, timestamp},
		{"BadMessage", []byte("yolo"), signature, *allowedSigner, namespace, timestamp},
	}
